...
/*
Program kept on crashing so I didn't keep the changes to the methods, comments will be added to say what should have been modified.
* */
...
public class MainActivity extends AppCompatActivity implements AddToDoFragment.OnDialogCloseListener, UpdateToDoFragment.OnUpdateDialogCloseListener{
    ...
    Spinner spinner; // get reference for our spinner
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //adding a spinner
        spinner = (Spinner) findViewById(R.id.todo_filter); // initialize the reference
        ArrayAdapter adapter = ArrayAdapter.createFromResource(this,R.array.todo_array,android.R.layout.simple_spinner_item);
        spinner.setAdapter(adapter); //set the adapter onto the spinner
    ...
    @Override
    //needed to add String choice and boolean status into the parameters
    public void closeDialog(int year, int month, int day, String description) {
        addToDo(db, description,formatDate(year, month, day));// needed to add choice and status
        cursor = getAllItems(db);
    ...
    private long addToDo(SQLiteDatabase db, String description,String duedate) {
        ...
        //cv.put(Contract.TABLE_TODO.COLUMN_NAME_CHOICE, choice);// would need choice and status  when adding a new todo inorder to sort
    ...
    private int updateToDo(SQLiteDatabase db, int year, int month, int day, String description,long id){
        ...
        ContentValues cv = new ContentValues();
        cv.put(Contract.TABLE_TODO.COLUMN_NAME_DESCRIPTION, description);
        //cv.put(Contract.TABLE_TODO.COLUMN_NAME_CHOICE,choice);//same as before but for updated to dos
        //cv.put(Contract.TABLE_TODO.COLUMN_NAME_STATUS,status);
        cv.put(Contract.TABLE_TODO.COLUMN_NAME_DUE_DATE, duedate);
    ...
    @Override
    //would need to add String choice and boolean status into the parameters and to updateToDo
